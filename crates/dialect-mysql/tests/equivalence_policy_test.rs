...
#[test]
fn strict_diff_reports_cast_alias_residual_but_mysql_policy_suppresses_it() {
    ...
    let mut current = vec![quantity_table(
        Expr::Raw("CAST('000' AS SIGNED INTEGER)".to_string()),
        Expr::Raw("quantity > 0".to_string()),
    )];
    ...
    let relaxed_ops = diff_with_config(&desired, &current, &mysql_diff_config(&dialect));
    assert!(
        relaxed_ops.is_empty(),
        "mysql equivalence policy should suppress cast/alias residual diffs",
    );
}

#[test]
fn strict_diff_reports_paren_and_whitespace_residuals_but_mysql_policy_suppresses_them() {
    let dialect = MysqlDialect;
    let mut desired = vec![quantity_table(
        Expr::Raw("((0))".to_string()),
        Expr::Raw("quantity    >     0".to_string()),
    ...
    assert!(
        relaxed_ops.is_empty(),
        "mysql equivalence policy should suppress paren/whitespace residual diffs",
    );
}

#[test]
fn mysql_policy_contract_is_symmetric_and_stable_and_keeps_structural_mismatch_strict() {
    let dialect = MysqlDialect;
    let policy = dialect.equivalence_policy();

    let cast_left = Expr::Raw("CAST('000' AS SIGNED INTEGER)".to_string());
    ...
    .expect("mysql equivalence policy must satisfy symmetry/stability contract");
...
fn normalize_all(dialect: &MysqlDialect, objects: &mut [SchemaObject]) {
...
fn mysql_diff_config(dialect: &MysqlDialect) -> DiffConfig {
    DiffConfig::new(
        false,
        Vec::new(),
