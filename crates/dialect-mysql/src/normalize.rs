use stateql_core::{
    CommentTarget, Expr, Function, Ident, IndexDef, IndexOwner, MaterializedView, PrivilegeObject,
    QualifiedName, SchemaObject, Sequence, Table, TypeDef, TypeKind, Value,
};
...
fn normalize_object_types(object: &mut SchemaObject) {
    match object {
        SchemaObject::Table(table) => {
            ...
            normalize_partition_hint(table);
        }
        SchemaObject::View(_) => {}
...
fn normalize_partition_hint(table: &mut Table) {
    if let Some(Value::String(partition_sql)) =
        table.options.extra.get_mut(extra_keys::TABLE_PARTITION_SQL)
    {
        *partition_sql = partition_sql.trim().to_string();
    }
}
...
mod types {
    ...
    fn normalize_custom_type(raw: &str) -> DataType {
        ...
        if canonical == "double" || canonical == "double precision" {
        ...
        if canonical == "datetime" || canonical == "timestamp" {
    ...
    fn normalize_custom(raw: &str) -> String {
        ...
        strip_integer_display_width(&canonical)
    }

    fn strip_integer_display_width(canonical: &str) -> String {
        let Some(open_paren) = canonical.find('(') else {
        ...
        let base = canonical[..open_paren].trim();
        if matches!(
            base,
