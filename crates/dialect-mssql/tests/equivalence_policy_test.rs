...
#[test]
fn strict_diff_reports_cast_alias_residual_but_mssql_policy_suppresses_it() {
    ...
    let relaxed_ops = diff_with_config(&desired, &current, &mssql_diff_config(&dialect));
    assert!(
        relaxed_ops.is_empty(),
        "mssql equivalence policy should suppress cast/alias residual diffs",
    );
}

#[test]
fn strict_diff_reports_paren_and_whitespace_residuals_but_mssql_policy_suppresses_them() {
    let dialect = MssqlDialect;
    let mut desired = vec![quantity_table(
        Expr::Raw("((0))".to_string()),
        Expr::Raw("quantity    >     0".to_string()),
    ...
    assert!(
        relaxed_ops.is_empty(),
        "mssql equivalence policy should suppress paren/whitespace residual diffs",
    );
}

#[test]
fn mssql_policy_contract_is_symmetric_and_stable_and_keeps_structural_mismatch_strict() {
    let dialect = MssqlDialect;
    ...
    .expect("mssql equivalence policy must satisfy symmetry/stability contract");
...
fn normalize_all(dialect: &MssqlDialect, objects: &mut [SchemaObject]) {
...
fn mssql_diff_config(dialect: &MssqlDialect) -> DiffConfig {
    DiffConfig::new(
        false,
        Vec::new(),
