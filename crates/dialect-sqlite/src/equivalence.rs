...
pub(crate) struct SqliteEquivalencePolicy;

pub(crate) static SQLITE_EQUIVALENCE_POLICY: SqliteEquivalencePolicy = SqliteEquivalencePolicy;

impl EquivalencePolicy for SqliteEquivalencePolicy {
...
fn normalize_integer_cast_literal(expr: &str) -> Option<String> {
    let (literal, data_type) = parse_cast_expression(expr)?;
...
fn parse_cast_expression(expr: &str) -> Option<(&str, &str)> {
    ...
    let inner = trimmed.get(5..trimmed.len() - 1)?;
    let separator_index = find_cast_as_separator(inner)?;
    let literal = inner[..separator_index].trim();
    let data_type = inner[separator_index + 4..].trim();
    Some((literal, data_type))
}

fn find_cast_as_separator(inner: &str) -> Option<usize> {
    ...
    while let Some((index, ch)) = chars.next() {
        ...
        if depth == 0 && lower[index..].starts_with(" as ") {
            return Some(index);
        }
    }
